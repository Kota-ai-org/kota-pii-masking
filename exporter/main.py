...
persisted watermark (oldest-first), DLP-masks the natural-language fields
(input/output) while keeping the whole trace intact (see deidentify_core), and
writes them as JSONL to the masked bucket in bounded chunks — advancing the
watermark after EACH chunk. The masked bucket is the only resource shared with
Kota; raw PII never lands in storage.
